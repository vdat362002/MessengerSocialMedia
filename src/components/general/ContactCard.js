  ...
  return (
    <div className={`flex items-center p-4 border-b-2 cursor-pointer ${selectContact._id === user._id ? 'bg-gray-100' : undefined} hover:bg-gray-100 transition-[background]`} onClick={handleSelectContact}>
      <div className='relative'>
        <Avatar src={user.profilePicture?.url || user.avatar?.url || Avt} alt={user.name} />
        {
      ...
      <div className='w-full ml-4'>
        <div className='flex items-center gap-3'>
          <p className='text-lg mb-1'>{user?.name ? user.name : (user.firstname && user.lastname) ? `${user.firstname} ${user.lastname}` : `@${user.username}`}</p>
