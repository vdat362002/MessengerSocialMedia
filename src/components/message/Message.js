    ...
    <div className={`mb-7 flex ${type === 'sender' ? 'items-end' : 'items-start'} ${type === 'sender' ? 'justify-end' : 'justify-start'} flex-col`}>
      {
        ...
        <div className={`flex ${type === 'sender' ? 'flex-row-reverse' : undefined}`}>
          <Avatar src={type === 'sender' ? (auth.user?.profilePicture?.url || Avt) : (recipientAvatar?.url ? recipientAvatar?.url : recipientAvatar || Avt)} size='30px' />
...

      {
        media.length > 0 &&
        <div className={`flex ${type === 'sender' ? 'flex-row-reverse' : undefined}`}>
          {!message && <Avatar src={type === 'sender' ? auth.user?.profilePicture?.url || Avt : recipientAvatar?.url} size='30px' />}
          <div className={`${type === 'sender' ? `${message ? '-translate-x-[50px]' : '-translate-x-3'}` : `${message ? 'translate-x-[50px]' : 'translate-x-3'}`}`}>
            {
              media.map((item, idx) => (
                <div className='relative'>
                  <img key={idx} src={item} alt={idx} className="w-40 h-auto rounded-md mb-2" />
                  {
                    sender._id === auth.user?._id &&
                    (
                      isRead
...

      {
        call &&
        <div className={`flex ${type === 'sender' ? 'flex-row-reverse' : undefined}`}>
          {!message && <Avatar src={type === 'sender' ? auth.user?.profilePicture?.url || Avt : recipientAvatar} size='30px' />}
          <div className={`${type === 'sender' ? `${message ? '-translate-x-[50px]' : '-translate-x-3'}` : `${message ? 'translate-x-[50px]' : 'translate-x-3'}`}`}>
            {
              call.video
                ? (
...

      {
        ...
        <div className={`flex ${type === 'sender' ? 'flex-row-reverse' : undefined} items-center`}>
          <Avatar src={type === 'sender' ? auth.user?.profilePicture?.url || Avt : recipientAvatar} size='30px' />
