    ...
    if (call.recipient !== auth.user._id || disconnect) {
      const chatData = {
        ...
        recipient: {
          ...
          profilePicture: call.recipientAvatar,
          username: call.recipientUsername,
        },
        text: '',
        media: [],
        audio: '',
    ...
    <>
      ...
      <div className={`bg-[rgba(0,0,0,.7)] fixed top-0 right-0 bottom-0 left-0 z-[999] p-5 flex items-center justify-center ${answer && call.video ? 'hidden' : 'block'}`}>
        <div className='bg-white w-full max-w-[350px] p-8 flex items-center justify-center flex-col rounded-lg shadow-2xl'>
          <Avatar src={call.recipientAvatar?.url || Avt} alt={call.recipientName || `@${call.recipientUsername}`} />
          <h1 className='font-medium text-xl mt-5'>{call.recipientName || `@${call.recipientUsername}`}</h1>
          {
            !answer &&
