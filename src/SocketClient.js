  ...
  useEffect(() => {
    socket.on('createMessageToClient', data => {
      ...
      const addConversationData = {
        ...
        senderName: data.sender?.name || `${data.sender?.firstname} ${data.sender?.lastname}`,
        senderAvatar: data.sender.profilePicture,
        recipientId: data.recipient._id,
        recipientUsername: data.recipient.username,
        recipientName: data.recipient?.name || `${data.recipient?.firstname} ${data.recipient?.lastname}`,
        recipientAvatar: data.recipient.profilePicture,
        media: data.media,
      ...
      spawnNotification(data.sender.name, data.sender.profilePicture?.url || Avt, 'https://web-inspace.herokuapp.com', 'Messenger')
    })
